...
contract ZipFarm is Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _zips, address _reflectionToken, uint256 _rewardPerBlock, bool _hasDividend) {
        zips = _zips;
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 zipsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(zipsReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.bonusEndBlock);
        uint256 zipsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(zipsReward.mul(1e12).div(lpSupply));
    ...
    function compoundReward(uint256 _pid) external payable nonReentrant {
        ...
        if(address(zips) != address(pool.lpToken)) {
            uint256 tokenAmt = pending / 2;
            uint256 tokenAmt0 = tokenAmt;
            address token0 = address(zips);
    ...
    function _calculateTotalStaked(uint256 _pid, IERC20 _lpToken, uint256 _amount, bool _deposit) internal {
        if(_deposit) {
            ...
            if(address(_lpToken) == address(zips)) {
    ...
    function availableRewardTokens() public view returns (uint256) {
        if(address(zips) == reflectionToken) return totalEarned;

        uint256 _amount = zips.balanceOf(address(this));
    ...
    // Safe zip transfer function, just in case if rounding error causes pool to not have enough zipss.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 zipsBal = zips.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > zipsBal) {
            transferSuccess = zips.transfer(_to, zipsBal);
        } else {
            transferSuccess = zips.transfer(_to, _amount);
        }
    ...
    //zips has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    ...
    function depositRewards(uint _amount) external nonReentrant {
        ...
        uint256 beforeAmt = zips.balanceOf(address(this));
        zips.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 afterAmt = zips.balanceOf(address(this));
    ...
    function emergencyWithdrawRewards(uint256 _amount) external onlyOwner {
        if(_amount == 0) {
            uint256 amount = zips.balanceOf(address(this));
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(_token != address(zips) && _token != reflectionToken, "cannot recover reward token or reflection token");
